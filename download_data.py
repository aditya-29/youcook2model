...
class DownloadData:
    ...
    #       -O {path} --no-check-certificate""".format(path=path)
            extract_command = "!tar -xf {path}".format(path=path)

        else:
            suffix = self.part.split("part")[-1]
            path = os.path.join(self.save_folder, "raw_videos.parta{suffix}".format(suffix))
            ...
            extract_command = "!tar -xf {path}".format(suffix=suffix, path=path)
    ...
    def _download_annotations(self):
        ...
        extract_command_train = "!tar -xf {train_path}".format(train_path=train_path)
        extract_command_test = "!tar -xf {test_path}".format(test_path = test_path)
